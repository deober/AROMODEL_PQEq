...
def Write_LAMMPS(f,nproc,In_File,Name):
	f.write('module load lammps\n\nsrun --cpu-bind-cores -np %d lammps -in %s -log log.%d' % (nproc,In_File,Name))

def Write_LAMMPS_Run_Only(f,nproc,In_File,Name):
	f.write('srun --cpu-bind-cores -np %d lammps -in %s -log log.%d' % (nproc,In_File,Name))

def Write_NWChem(f,nproc,In_File,constraint='cori'):
	if constraint != 'knl':
		f.write('module load nwchem\nsrun --cpu-bind-cores -np %d nwchem %s' % (nproc,In_File))
	else:
		f.write('module load nwchem\nsrun --cpu-bind-cores -S 4 -n 64 -c 4 %d nwchem %s' % (nproc,In_File))
	return

def Write_SLURM(File_Name,In_File,Name,nproc,Cluster_Location,Job_Type,queue="shared",proc_per_node=32,account = "m3047",walltime = 2,Executable_Path = "",OMP_Path = "",constraint='cori',nodes=1):
	f = open(File_Name,'w')
	...
	if nodes == 0:
		...
		queue = "compute"
		ppn = proc_per_node"""
	f.write('#!/bin/bash\n#SBATCH --job-name="%s"\n#SBATCH --output=%s\n#SBATCH --partition=%s\n#SBATCH --nodes=%d\n#SBATCH --ntasks-per-node=%d\n#SBATCH -A %s\n#SBATCH --export=ALL\n#SBATCH -t %d:00:00\n\ncd %s\n' % (Name,Name,queue,nodes,proc_per_node,account,walltime,Cluster_Location))
	if Job_Type == "QChem":	
		if Executable_Path == "":
